...
const NavItem = ({ className }: { className?: string }) => {
  ...
  const filteredAuthenticatedLinks = authenticatedLinks.filter((link) => {
    // Hide the "X" link if the role is 'student'
    if (role === "Student" && link.name === "X") {
  ...
  if (!isDesktop) {
    return (
      <div className={`w-full flex h-screen rounded-xl  gap-2 ${className}`}>
            ...
            <Link
              ...
              className={`w-full font-bold py-2 px-4 text-2xl ${
                currentPath === link.link
                  ...
                  : ""
              } hover:underline rounded text-black dark:text-black transition duration-500 ease-in-out flex justify-center gap-3 md:justify-start`}
            >
              {link.name}
            </Link>
          ))}
